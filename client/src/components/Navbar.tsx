...
const Navbar = () => {
  ...
  useEffect(() => {
    ...
    const shown = sessionStorage.getItem("loginGreetingShown");

    if (!shown) {
      toast(`${getTimeGreeting()}, ${session.user.name} 👋`, {
        description: "Welcome back to Ai Site Builder 🚀",
      });
  ...
  useEffect(() => {
    if (!session?.user) {
      // User logged out → reset greeting flag
      ...
      <nav className="z-50 flex items-center justify-between w-full py-4 px-4 md:px-16 lg:px-24 xl:px-32 backdrop-blur border-b text-white border-slate-800">
        ...
        <div className="flex items-center gap-3">
          {!session?.user ? (
            ...
            <>
              {/* Greeting */}
              <div className="hidden sm:flex items-center text-sm text-gray-300 mr-2">
                <span className="text-slate-400">{getTimeGreeting()},</span>
