...
interface SidebarProps {
  ...
  //                        code updated <br/> 
  //                        <span className='text-xs text-gray-500 font-normal'>{new Date(ver.timestamp).toLocaleString()}</span>
  //                       </div>
  ...
  //           <button disabled={isGenerating || !input.trim()} className='absolute bottom-2.5 right-2.5 rounded-full bg-linear-to-r from-indigo-500 to-indigo-600 hover:from-indigo-600 hover:to-indigo-700 text-white transition-colors disabled:opacity-50' >
  //             {isGenerating ? (
          ...
          if (isMessage) {
            ...
            return (
              ...
              >
                ...
                <div
                  className={`max-w-[80%] p-2 px-4 rounded-2xl shadow-sm text-sm leading-relaxed ${
                    isUser
    ...
    >
      <div className="relative">
        <textarea
          ...
          className="w-full p-3 pr-12 rounded-xl resize-none text-sm outline-none ring ring-gray-700 focus:ring-indigo-500 bg-gray-800 text-gray-100 placeholder-gray-400 transition-all"
          disabled={isGenerating}
        />

        <button
          disabled={isGenerating || !input.trim()}
          className="absolute bottom-3 right-3 rounded-full bg-gradient-to-r from-indigo-500 to-indigo-600 hover:from-indigo-600 hover:to-indigo-700 text-white transition-colors disabled:opacity-50"
        >
          {isGenerating ? (
            <Loader2Icon className="size-7 p-1.5 animate-spin text-white" />
          ) : (
